...
function start_kou_shortcut_remapping {
    ...
    if [ "$HOSTNAME" = "max-B9440UA" ]; then
        ...
        tmux new-session -d 'sudo /usr/local/sbin/kou_maxs /dev/input/by-path/platform-i8042-serio-0-event-kbd keyb >> /home/max/kou_log_new.txt'
        ...
        tmux new-session -d 'sudo /usr/local/sbin/kou_maxs /dev/input/by-id/usb-Logitech_USB_Receiver-if02-event-kbd Logi >> /home/max/kou_log_new.txt'
        tmux new-session -d 'sudo /usr/local/sbin/kou_maxs /dev/input/by-id/usb-Telink_Wireless_Receiver-if01-event-kbd Telink >> /home/max/kou_log_new.txt'
