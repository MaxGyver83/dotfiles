...
if [ -z "$pid" ] ; then
  ...
  WINIT_X11_SCALE_FACTOR=1.0 ~/install/alacritty-0.7.1 -t Cheatsheet -o "window.dimensions.columns=$cols" -o "window.dimensions.lines=$rows" -e bash -c 'cat ~/bin/cheatsheet.txt && read -rsn1'
else
  kill $pid
fi
